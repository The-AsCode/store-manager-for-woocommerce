...
const Pagination = () => {
	...
	const handleNext = () => {
		if ( data?.max_num_pages === 0 || data?.max_num_pages === query.page )
			return;
	...
	return (
		<div className="wmx-bg-white wmx-py-2 wmx-px-4 wmx-border-y wmx-flex wmx-justify-between wmx-items-center">
			{ data && data?.total_products > 0 ? (
				<p>
					Showing { query.page * query.per_page - query.per_page + 1 }{ ' ' }
					-{ ' ' }
					{ query.page * query.per_page > data?.total_products
						? data?.total_products
						: query.page * query.per_page }{ ' ' }
					of { data?.total_products } Products
				</p>
			) : (
			...
			<div className=" wmx-flex wmx-items-center">
				...
				<button
					disabled={
						data?.max_num_pages === 0 ||
						data?.max_num_pages === query.page
					}
					onClick={ handleNext }
