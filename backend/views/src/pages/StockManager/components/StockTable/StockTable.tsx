...
const StockTable = () => {
	...
	const [ updateStockAction, { isLoading: togglerStatusLoading } ] =
		useUpdateStockActionMutation();

	const handleStockAction = async (
		stockStatus: string,
		id: number,
		wcStock: string | boolean
	) => {
		...
		if ( result ) {
			toast.success( 'Stock Action Updated' );
		}
	};

	return (
		<div className="wmx-mt-4">
			<div className="wmx-inline-block wmx-min-w-full wmx-align-middle px-8">
				<Table>
					<thead className="wmx-bg-white ">
						<tr>
							<Table.Heading className="wmx-pl-4 wmx-w-20">
								Status
							</Table.Heading>
							...
							<Table.Heading className="wmx-w-48">
								Stock/Backorder
							</Table.Heading>
						</tr>
					</thead>
										...
										<tr key={ product.id }>
											...
											<Table.Data>
												<img
													...
													alt={ product.name }
													className="wmx-h-7 wmx-w-7"
												/>
											</Table.Data>
											<Table.Data>
												{ product.name }
											</Table.Data>
											<Table.Data className="wmx-capitalize">
											...
											product.variations.map(
												( variation ) => (
													<tr key={ variation.id }>
														...
														<Table.Data className="wmx-capitalize wmx-bg-gray-100">
															{
